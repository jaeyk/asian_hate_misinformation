...
# Load us dataframe
load(here('processed_data_hate', 'processed_data_hate', 'df_ready.Rdata'))

# Load world data
parsed <- readRDS(here('processed_data_hate', 'processed_data_hate', 'parsed.rds'))
...
ggsave('mention wuhan label.png')
...
df_usa %>%
  ...
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) 
ggsave('mention china virus label.png')
...
df_usa %>%
  ...
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) 
ggsave('mention kung flu label.png')

## Count of virus mentions over time

### Mutate date format
df_usa <- df_usa %>% 
  mutate(created_at_date = as.Date(paste(str_sub(created_at,
                                                 start = 5,
                                                 end = 10), 
                                         "2020"),
                                   format = "%b %d %Y"))
...
df_usa %>%
  ...
  theme_clean()
ggsave('wuhan tweets prepost.png')
...
df_usa %>%
  ...
  ylab('Count of Tweets') +
  theme_clean()
ggsave('china virus tweets prepost.png')
...
df_usa %>%
  ...
  ylab('Count of Tweets') +
  theme_clean()
ggsave('kung flu tweets prepost.png')
...
df_usa %>%
  ...
  count(wuhan_virus) %>%
  ggplot() +
  geom_point(aes(x = created_at_date, 
                 y = n,
                 color = Label)) +
  geom_line(aes(x = created_at_date,
                y = n, 
                color = Label)) +
  geom_vline(xintercept = as.Date("March 16 2020", 
                                  format = "%b %d %Y")) +
  ...
  ylab('Count') +
  theme_clean()
ggsave('wuhan tweets prepost label.png')
...
df_usa %>%
  ...
  theme_clean() +
  facet_wrap(~Label)
ggsave('china virus tweets prepost label.png')
...
df_usa %>%
  ...
  ylab('Count') +
  theme_clean()
ggsave('kung flu tweets prepost label.png')
...
p <- ggplot()
p <- p + geom_polygon( data=wuhan_tweets_by_state, 
                       aes(x=long, y=lat, group=group, fill = n), 
                       color="white", size = 0.2) 
p
ggsave('map of wuhan tweets.png')

# Comparative Plots

parsed <- parsed %>%
  mutate(chinese_virus = as.numeric(str_detect(full_text, "chinavirus|china virus|chinese flu|chineseflu|chinese virus|chinesevirus")),
...
parsed %>%
  ...
  theme_clean() +
  facet_wrap(~usa_tweet)
ggsave('chinese virus comparison.png')
...
parsed %>%
  ...
  theme_clean() +
  facet_wrap(~usa_tweet)
ggsave('kung flu comparison.png')
...
ggplot() + 
  geom_point(data = bat_soup,
             aes(x = created_at_date,
                 y = n),
             color = 'blue') +
  geom_line(data = bat_soup,
            aes(x = created_at_date,
                y = n),
            color = 'blue') +
  geom_point(data = propaganda,
             aes(x = created_at_date,
                 y = n),
             color = 'red',
             show.legend = TRUE) +
  geom_line(data = propaganda,
            aes(x = created_at_date,
                y = n),
            color = 'red') +
  geom_vline(xintercept = as.Date("March 16 2020", 
...
ggsave('intl comparison wuhan.png')
...
parsed %>%
  ...
  theme_clean() +
  facet_wrap(~country_code)
ggsave('intl comparison chinese virus.png')
...
parsed %>%
  ...
  theme_clean() +
  facet_wrap(~country_code)
ggsave('intl comparison kung flu.png')