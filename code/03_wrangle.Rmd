...
```{r}
# Add date variable 
df$date <- as.POSIXct(df[["created_at"]], tz = "UTC", format = "%a %b %d %H:%M:%S %z %Y") %>% word(1)
...
# Preprocess text 
df$clean_text <- clean_text(df$full_text)
...
```{r}
save(df, df_usa, file = here("processed_data", "df_ready.Rdata"))
```