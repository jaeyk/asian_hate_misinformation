...
```{r}
corpus$full_text[str_detect(corpus$full_text, "selling")][1:20]
```
...
corpus %>%
  ...
  summarize(mean_bat = mean(bat),
            mean_propanda = mean(propaganda)) %>%
  pivot_longer(contains("mean"),
               "type") %>%
  ggplot(aes(x = trump_speech, y = value, fill = Label)) +
