...
df <- df %>%
    mutate(full_text = str_to_lower(full_text),
           ...
           bat_soup = 
as.numeric(str_detect(full_text, "bat soup")),
           racism = 
as.numeric(str_detect(full_text, "calling corona|calling covid")))
```
...
twitter <- bind_rows(
    ...
    mutate(add_normalized(df, bat_soup), Terms = "Bat soup"),
    mutate(add_normalized(df,
racism), Terms = "Calling COVID"))
```
...
twitter <- twitter %>%
  pivot_longer(matches("virus|flu|bat|calling"),
               names_to = "Terms", 
               values_to = "rescaled")
```
...
twitter_plot 
ggsave(here("outputs", "desc_plot.png"), width = 10)
```
